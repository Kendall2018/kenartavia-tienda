...
public interface ProductoRepository extends JpaRepository<Producto, Integer> {
    ...
    //Práctica #2: consulta ampliada
    @Query(value = "SELECT p FROM Producto p WHERE p.precio BETWEEN :precioInf AND :precioSup AND p.existencias >= :existencias ORDER BY p.existencias DESC, p.precio ASC")
    public List<Producto> consultaAmpliada(@Param("precioInf") double precioInf,
            @Param("precioSup") double precioSup,
            @Param("existencias") int existencias);
